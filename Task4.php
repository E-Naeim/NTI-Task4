...
// Allowed Extension
$allowed = ['pdf', 'docx'];
$ex_type = explode('/', $file_type); // extract the extension type from the uploaded file

// Check if uploaded extension is allowed
if(in_array($ex_type[1],$allowed)){
    $finalName = 'random'.rand(1,50).'time'.time().'.'.$ex_type[1]; // to not repeat name
    $path = './uploaded'.$finalName;
    move_uploaded_file($file_tmp,$path); // move uploaded file
}else{
    echo 'Not a valid CV extension, only PDF and DOCX are accepted!';
}
...
$_SESSION['Profile'] = [$name, $password, $email, $address, $gender, $url, $finalName];
};
